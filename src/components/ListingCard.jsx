    ...
    >
        ...
        <div style={{ position: "absolute", top: 12, right: 12, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {onQuickLook && (
            <button
              ...
              aria-label={`Quick look at ${listing.title}`}
              style={{
                width: 32, height: 32, borderRadius: "50%",
      ...
      <div className="fs-card-body">
        ...
        <dl className="fs-card-specs">
          <div className="fs-card-specs-row"><dt>Total time</dt><dd>{hasTT ? formatHours(listing.ttaf) : '—'}</dd></div>
          <div className="fs-card-specs-row"><dt>Engine SMOH</dt><dd>{hasSMOH ? formatHours(listing.eng_hours) : '—'}</dd></div>
          <div className="fs-card-specs-row"><dt>IFR</dt><dd>{listing.ifr ? '✓' : '—'}</dd></div>
          <div className="fs-card-specs-row"><dt>Glass cockpit</dt><dd>{listing.glass_cockpit ? '✓' : '—'}</dd></div>
        </dl>
        <div className="fs-card-dealer">
          {dealerName ? (
            <>
              {dealerVerified && Icons.shield}
              ...
              {listing.seller?.abn_verified_at && (
                <span className="fs-card-abn-pill" title="Business ABN verified against ABR public data">ABR</span>
              )}
              {location && <><span className="fs-card-dealer-sep">·</span><span className="fs-card-dealer-loc">{location}</span></>}
