...
.fs-nav-link.active { color: var(--fs-gray-900); font-weight: 600; }
...
.fs-nav-btn-primary { background: var(--fs-gray-900); color: white; }
.fs-nav-btn-primary:hover { background: var(--fs-gray-800); }
...
.fs-hero h1 {
  font-family: var(--fs-font); font-size: clamp(44px, 5.5vw, 72px); color: white;
  line-height: 0.95; margin-bottom: 16px; font-weight: 800; letter-spacing: -0.04em;
}
.fs-hero h1 em { font-style: normal; color: white; }
.fs-hero-sub {
  color: rgba(255,255,255,0.6); font-size: 18px; margin-bottom: 40px;
  max-width: 500px; margin-left: auto; margin-right: auto; font-weight: 400;
}
@media (max-width: 640px) {
  .fs-hero { padding: 48px 0 56px; }
  .fs-hero h1 { font-size: 34px; }
  .fs-hero-sub { font-size: 15px; margin-bottom: 28px; }
}
...
.fs-search-label {
  ...
  letter-spacing: 0.8px; color: var(--fs-gray-400); margin-bottom: 4px;
}
...
.fs-search-btn {
  ...
  padding: 0 28px; cursor: pointer; display: flex;
...
@media (max-width: 700px) {
  .fs-search-bar { flex-direction: column; border-radius: var(--fs-radius); }
  .fs-search-field { border-right: none; border-bottom: 1px solid var(--fs-gray-100); padding: 12px 16px; }
  .fs-search-btn { padding: 14px; }
}
...
.fs-stat-num { color: white; font-size: 26px; font-weight: 700; line-height: 1.2; }
.fs-stat-label { color: rgba(255,255,255,0.5); font-size: 12px; font-weight: 500; margin-top: 2px; }
...
.fs-detail-cta-primary {
  background: var(--fs-gray-900); color: white; margin-bottom: 10px;
}
.fs-detail-cta-primary:hover { background: var(--fs-gray-700); }
...
.fs-search-inline-input:focus { border-color: var(--fs-blue); }
.fs-filter-chip {
    ...
    <>
      ...
      <section className="fs-hero">
        <div className="fs-container fs-hero-content">
          <h1>Australia's Aircraft<br/><em>Marketplace</em></h1>
          <p className="fs-hero-sub">
            Buy and sell aircraft with confidence. From trainers to turboprops — find your next aircraft here.
          </p>
          ...
          <div className="fs-search-bar">
            ...
            <div className="fs-search-field">
              <span className="fs-search-label">Manufacturer</span>
          ...
          <div className="fs-stats">
            ...
            <div className="fs-stat"><div className="fs-stat-num">$48M+</div><div className="fs-stat-label">Aircraft Value Listed</div></div>
          </div>
        </div>
      </section>
...
const BuyPage = ({ setSelectedListing, savedIds, onSave }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return results;
  }, [search, sortBy, catFilter, stateFilter]);
      ...
      <div className="fs-search-page-bar">
        <div className="fs-container fs-search-page-inner">
          <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
            <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--fs-gray-400)" }}>{Icons.search}</span>
            <input className="fs-search-inline-input" placeholder="Search aircraft..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <select className="fs-filter-chip" value={catFilter} onChange={e => setCatFilter(e.target.value)} style={{ minWidth: 150 }}>
            <option value="">All Categories</option>
            {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          <select className="fs-filter-chip" value={stateFilter} onChange={e => setStateFilter(e.target.value)}>
            <option value="">All States</option>
            {STATES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
      ...
      <section className="fs-section">
        <div className="fs-container">
          <div className="fs-results-bar">
            <span className="fs-results-count">{filtered.length} aircraft found</span>
          ...
          {filtered.length === 0 ? (
            <div className="fs-empty">
              ...
              <p>No aircraft match your search. Try broadening your filters.</p>
            </div>
          ) : (
            <div className="fs-grid">
              {filtered.map(l => (
