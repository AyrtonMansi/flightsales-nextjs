...
export class SynapseClient {
    ...
    async createJob(params) {
        ...
        if (!validated.success) {
            return this.createErrorResponse('INVALID_REQUEST', validated.error.message, validated.error.flatten());
        }
    ...
    async listNodes(options = {}) {
        ...
        if (options.minReputation)
            params.set('minReputation', String(options.minReputation));
        if (options.limit)
            params.set('limit', String(Math.min(options.limit, 100))); // Max 100
        try {
            ...
            return {
                success: true,
                data: data.nodes.map((n) => this.deserializeNode(n)),
                requestId,
                timestamp: new Date(),
            };
        }
     ...
     */
    async fetchWithRetry(path, options, requestId, attempt = 1) {
