...
export class SynapseContracts {
    ...
    token;
    verifier;
    constructor(provider, contractAddresses, signer) {
        this.provider = provider;
        this.signer = signer;
        // AUDIT: Contract instances are immutable after creation
        this.jobRegistry = new ethers.Contract(contractAddresses.jobRegistry, JOB_REGISTRY_ABI, signer || provider);
        this.token = new ethers.Contract(contractAddresses.token, TOKEN_ABI, signer || provider);
        this.verifier = new ethers.Contract(contractAddresses.verifier, ZK_VERIFIER_ABI, provider // Verifier is read-only from client side
        );
    }
    // ============================================================================
    ...
    async createJob(job, options = {}) {
        ...
        if (balance < maxPriceWei) {
            throw new ContractError(`Insufficient HSK balance. Have: ${ethers.formatUnits(balance, 18)}, Need: ${job.maxPrice}`);
        }
        // Check allowance
        const allowance = await this.token.allowance(address, await this.jobRegistry.getAddress());
        if (allowance < maxPriceWei) {
            throw new ContractError('Insufficient token allowance. Approve the job registry first.');
        }
        ...
        if (maxPriceUsd > 10000) {
            throw new ContractError(`Max price ${maxPriceUsd} exceeds safety limit. Contact support for large jobs.`);
        }
        const tx = await this.jobRegistry.createJob(ethers.keccak256(ethers.toUtf8Bytes(job.id)), job.modelId, maxPriceWei, this.priorityToUint(job.priority), {
    ...
    async assignJob(jobId, options = {}) {
        ...
        const tx = await this.jobRegistry.assignJob(ethers.keccak256(ethers.toUtf8Bytes(jobId)), nodeAddress, {
     ...
     */
    async completeJob(jobId, result, options = {}) {
        if (!this.signer) {
            throw new ContractError('Signer required for write operations');
        }
        ...
        const proofBytes = ethers.AbiCoder.defaultAbiCoder().encode(['bytes', 'uint256[]'], [result.proof.proof, result.proof.publicInputs.map(x => BigInt(x))]);
        const tx = await this.jobRegistry.completeJob(ethers.keccak256(ethers.toUtf8Bytes(jobId)), proofBytes, {
     ...
     */
    async disputeJob(jobId, options = {}) {
        if (!this.signer) {
            throw new ContractError('Signer required for write operations');
        }
        const tx = await this.jobRegistry.disputeJob(ethers.keccak256(ethers.toUtf8Bytes(jobId)), {
     ...
     */
    async releasePayment(jobId, options = {}) {
        if (!this.signer) {
            throw new ContractError('Signer required for write operations');
        }
        const tx = await this.jobRegistry.releasePayment(ethers.keccak256(ethers.toUtf8Bytes(jobId)), {
    ...
    async getJobStatus(jobId) {
        const statusUint = await this.jobRegistry.getJobStatus(ethers.keccak256(ethers.toUtf8Bytes(jobId)));
    ...
    async getEscrow(jobId) {
        const [amount, status, expiry] = await this.jobRegistry.getEscrow(ethers.keccak256(ethers.toUtf8Bytes(jobId)));
        const statusMap = {
            0: 'locked',
            1: 'released',
            2: 'refunded',
    ...
    async verifyProof(proof) {
        try {
            const result = await this.verifier.verifyProof(proof.proof, proof.publicInputs.map(x => BigInt(x)));
            return result;
        }
        catch (error) {
     ...
     */
    async approveTokens(amount, options = {}) {
        if (!this.signer) {
            throw new ContractError('Signer required for write operations');
        }
    ...
    onJobCreated(callback) {
        const filter = this.jobRegistry.filters.JobCreated();
        const handler = (log) => {
